...
#if 0
#define WISKY_PRINTK printk
#else
#define WISKY_PRINTK pr_debug
#endif
			...
			struct snd_ctl_elem_value *ucontrol)
{
	...
	if (ucontrol->value.integer.value[0] &&
		...
		//wisky-lxh@20151228
		#ifdef USE_EXT_AMP
		//gpio_set_value(gpio_spk_en, 1);
		gpio_direction_output(gpio_spk_en, 1);
		// printk("wisky_spk gpio_spk_en = 1");
		#endif
		//end-wisky-lxh	
	} else if (!ucontrol->value.integer.value[0] &&
		   codec_data->device_power[MT6391_DEV_OUT_SPEAKERL]) {
		...
		//wisky-lxh@20151228
		#ifdef USE_EXT_AMP
		//gpio_set_value(gpio_spk_en, 0);
		gpio_direction_output(gpio_spk_en, 0);
		// printk("wisky_spk gpio_spk_en = 0");
		#endif
		//end-wisky-lxh	
	}
...
static int mt6391_dev_probe(struct platform_device *pdev)
{
	...
	if (node){
		//wisky-lxh@20151228
		 ...
		 ret = gpio_request_one(gpio_spk_en, GPIOF_OUT_INIT_LOW,"extspkamp-gpio");
		 if (ret < 0) {
				printk("Unable to request gpio extspkamp-gpio\n");
				return -EINVAL;
		 }
		 printk("[Accdet]gpio_spk_en=%d!\n",gpio_spk_en);
		 #endif
	}
	/*
	//wisky-lxh@20151228
	#ifdef USE_EXT_AMP
	gpio_spk_en = of_get_named_gpio(dev->of_node, "extspkamp-gpio", 0);
	//ret = gpio_request_one(gpio_spk_en, GPIOF_OUT_INIT_LOW,"extspkamp-gpio");
	ret = gpio_request(gpio_spk_en,"extspkamp-gpio");
	if (ret < 0) {
		printk("Unable to request gpio extspkamp-gpio\n");
		return -EINVAL;
	}
