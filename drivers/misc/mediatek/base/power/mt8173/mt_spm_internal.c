...
void __spm_reset_and_init_pcm(const struct pcm_desc *pcmdesc)
{
	...
	spm_write(SPM_PCM_CON1, con1 | CON1_CFG_KEY | CON1_EVENT_LOCK_EN |
		  ...
		  (pcmdesc->replace ? 0 : CON1_IM_NONRP_EN) | CON1_MIF_APBEN);
}
...
void __spm_set_power_control(const struct pwr_ctrl *pwrctrl)
{
	...
	spm_write(SPM_AP_STANBY_CON, (!pwrctrl->md32_req_mask << 21) |
	...
	if (pwrctrl->param1 == 0)
		spm_write(SPM_PCM_SRC_REQ, (!!pwrctrl->pcm_apsrc_req << 0));
				       ...
				       const struct pcm_desc *pcmdesc, bool suspend)
{
	...
	wake_reason_t wr = WR_UNKNOWN;
	 ...
	 // printk("w100 __spm_output_wake_reason\n");
	...
	if ((wakesta->event_reg & 0x100000) == 0) {
		spm_print(suspend, "pcm sleep abort!\n");
		wr = WR_PCM_ABORT;
	}
...
MODULE_DESCRIPTION("SPM-Internal Driver v1.0");
