...
static struct msa_i2c_data *msa_obj;
...
Return: 0
*************************************************/
static int msa_readCalibration(struct i2c_client *client, int dat[MSA_AXES_NUM])
{
    struct msa_i2c_data *obj = i2c_get_clientdata(client);
...
/*----------------------------------------------------------------------------*/
static int msa_misc_open(struct inode *inode, struct file *file)
{
	file->private_data = msa_obj;
 ...
 *** mc3xxx_release
 *****************************************/
static int msa_misc_release(struct inode *inode, struct file *file)
{
...
#ifdef CONFIG_COMPAT
static long msa_misc_compat_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	long err = 0;
...
static const struct file_operations msa_misc_fops = {
        .owner = THIS_MODULE,
		.open			= msa_misc_open,
		.release		= msa_misc_release,
        .unlocked_ioctl = msa_misc_ioctl,
#ifdef CONFIG_COMPAT
		.compat_ioctl = msa_misc_compat_ioctl,
#endif
};
...
static ssize_t msa_enable_show(struct device_driver *ddri, char *buf)
{
	...
	ret = msa_get_enable(msa_handle, (char *)(&bEnable));
    if (ret < 0){
        ret = -EINVAL;
    }
    else{
...
static DRIVER_ATTR(axis_data,       S_IRUGO | S_IWUSR,  msa_axis_data_show,          NULL);
...
static DRIVER_ATTR(primary_offset,  S_IWUSR | S_IRUGO,  msa_primary_offset_show,     NULL);
static DRIVER_ATTR(vendor,          S_IWUSR | S_IRUGO,  msa_vendor_show,             NULL);
static DRIVER_ATTR(version,         S_IWUSR | S_IRUGO,  msa_version_show,            NULL); 
#if MSA_OFFSET_TEMP_SOLUTION
static DRIVER_ATTR(offset,          S_IWUSR | S_IRUGO,  msa_offset_show,             NULL);
static DRIVER_ATTR(calibrate_msaGSensor,       S_IWUSR | S_IRUGO,  msa_calibrate_show,          msa_calibrate_store);
#endif
#if FILTER_AVERAGE_ENHANCE
static DRIVER_ATTR(average_enhance, S_IWUSR | S_IRUGO,  msa_average_enhance_show,    msa_average_enhance_store);
#endif 
/*----------------------------------------------------------------------------*/
...
static int msa_enable_nodata(int en)
{
	int err = 0;
	int value = 0;
	struct msa_i2c_data *obj = msa_obj;

	if (NULL == obj) {
		MI_ERR("msa_obj is null!\n");
		return -1;
	}
	value = en;
	...
	if (((value == 0) && (sensor_power == false)) || ((value == 1) && (sensor_power == true)))	{
		...
		err = msa_setPowerMode(obj->client, sensor_enable_status);
	}
...
static int msa_set_delay(u64 ns)
{
	...
	err = msa_set_odr(obj->client, value);
	if (err) {
...
static int msa_get_data(int *x, int *y, int *z, int *status)
{
	...
	err = msa_readSensorData(obj->client, buff);
	if (err < 0) {
		MI_ERR("%s call msa_readSensorData failed!err=%d\n", __func__, err);
	} else {
...
static int msa_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	printk("msa300 client->addr=0x%x\n",client->addr);
	msa_obj = obj;
	obj->hw = hw;
	...
	res = msa_resetCalibration(client);
	if (res != 0) {
		return res;
	}
...
#endif
	res = i2c_smbus_read((PLAT_HANDLE) client, NSA_REG_WHO_AM_I, &chip_id);	
	if(chip_id != 0x13){
		client->addr = 0x27;
		res = i2c_smbus_read((PLAT_HANDLE) client, NSA_REG_WHO_AM_I, &chip_id);	
	}	
	if(chip_id !=0x13)
	{
		MI_ERR("msa300 read chip id fail!\n");
		goto exit_init_failed;  
	}
...
#if MTK_AUTO_MODE   
		res = msa_create_attr(&(msa_init_info.platform_diver_addr->driver)); 
#else
	res = msa_create_attr(&msa_gsensor_driver.driver);
#endif    
	if (res < 0) {
...
{    
	const char *name = "mediatek,msa300";
	MI_FUN;
