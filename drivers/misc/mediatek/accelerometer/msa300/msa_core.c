...
#define MSA_REG_PAGE(REG)                (((REG)>>8)&0xFF)
...
#define MSA_NSA_INIT_SECTION                            { NSA_REG_G_RANGE,              0xc0,   0x03    },                                  \
...
int msa_register_read(MSA_HANDLE handle, short addr, unsigned char *data)
{
    ...
    /* check page */
    if(MSA_REG_PAGE(addr) > 0) {
        res = msa_gsensor_drv.method->smi.read(handle, 0x0, &cur_page);
        if(res != 0) {
            return res;
        }

        if(cur_page != MSA_REG_PAGE(addr)) {
            res |= msa_gsensor_drv.method->smi.write(handle, 0x0, MSA_REG_PAGE(addr));
            if(res != 0) {
                return res;
            }
        }
    ...
    if(MSA_REG_PAGE(addr) > 0) {
        /* restore page NO. */
        res |= msa_gsensor_drv.method->smi.write(handle, 0x0, cur_page);
    }
