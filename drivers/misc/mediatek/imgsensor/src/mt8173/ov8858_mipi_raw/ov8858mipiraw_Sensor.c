...
static imgsensor_info_struct imgsensor_info = { 
	...
	.pre = {
		.pclk = 74400000, //148800000,			//record different mode's pclk
		...
		.mipi_data_lp2hs_settle_dc = 85, //23
		/*	 following for GetDefaultFramerateByScenario()	*/
		.max_framerate = 300,	
	},
	.cap = {
		.pclk = 74400000,              //hrx148800000
		.linelength  = 1940,
		.framelength = 2556,
		.startx = 0,
		.starty = 0,
	...
	.hs_video = {
		...
		/*	 following for  MIPIDataLowPwr2HighSpeedSettleDelayCount by different scenario	*/
		.mipi_data_lp2hs_settle_dc = 85, //30
		/*	 following for GetDefaultFramerateByScenario()	*/
		.max_framerate = 1200,
	},
	.slim_video = {
	...
	.mipi_lane_num = SENSOR_MIPI_2_LANE,//mipi lane num  //hrx4
	.i2c_addr_table = {0x42, 0xff},//record sensor support all write id addr, only supprt 4must end with 0xff
};


static imgsensor_struct imgsensor = {
	.mirror = IMAGE_NORMAL,				//mirrorflip information  IMAGE_NORMAL
...
static SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[5] =	 
 //{ 3296, 2480,	  12,	12, 3272, 2456, 1636, 1228,   4,	2, 1632, 1224,	 0, 0, 1632, 1224}, // Preview 
{{ 3296, 2480,	  12,	12, 3272, 2456, 1636, 1228,   4,	2, 1632, 1224,	 0, 0, 1632, 1224}, // Preview 
 ...
 { 3296, 2480,	  12,	12, 3272, 2456, 1632, 1228,   4,	2, 1632, 1224,	 0, 0, 1632, 1224}};// slim video 
...
static kal_uint16 set_gain(kal_uint16 gain)
{
	...
	if (gain < BASEGAIN || gain > 11 * BASEGAIN) {
		if (gain < BASEGAIN)
			gain = BASEGAIN;
		else if (gain > 11 * BASEGAIN)
			gain = 11 * BASEGAIN;		 
	}
...
//  ;;
	...
	write_cmos_sensor(0x302 , 0x1c);  //le
	...
	write_cmos_sensor(0x3011, 0x60);  //hrx2015-3-12
	...
	write_cmos_sensor(0x3018, 0x32);  //hrx0x72
	...
	write_cmos_sensor(0x4837, 0x17);   //hrx0x16
...
//  100 99 3264 2448);    //
	write_cmos_sensor(0x100 , 0x00 );  //  hrx00
	write_cmos_sensor(0x30e , 0x02);  //   hrx00
	write_cmos_sensor(0x312 , 0x03);  //   hrx01
	write_cmos_sensor(0x3015, 0x00);  // hrx01 
	write_cmos_sensor(0x3700, 0x18);  //30
	write_cmos_sensor(0x3701, 0x0c);  //18  
	write_cmos_sensor(0x3702, 0x28);  // 50 
	write_cmos_sensor(0x3703, 0x19);  //32  
	write_cmos_sensor(0x3704, 0x14);  //28  
	write_cmos_sensor(0x3707, 0x04);  // 08
	write_cmos_sensor(0x3708, 0x24);  // 48 
	write_cmos_sensor(0x3709, 0x33);  // 66
	write_cmos_sensor(0x370c, 0x04);  //  07
	write_cmos_sensor(0x3718, 0x12);  // 14 
	write_cmos_sensor(0x3712, 0x42);  // 44
	write_cmos_sensor(0x371e, 0x19);  //  31
	write_cmos_sensor(0x371f, 0x40);  // 7f 
	write_cmos_sensor(0x3720, 0x05);  // 0a 
	write_cmos_sensor(0x3721, 0x05);  // 
	write_cmos_sensor(0x3724, 0x06);  // 0c 
	write_cmos_sensor(0x3725, 0x01);  // 02 
	write_cmos_sensor(0x3726, 0x06);  //  0c
	write_cmos_sensor(0x3728, 0x05);  //  0a
	write_cmos_sensor(0x3729, 0x02);  //  03
	write_cmos_sensor(0x372a, 0x03);  //  06
	write_cmos_sensor(0x372b, 0x53);  //  a6
	write_cmos_sensor(0x372c, 0xa3);  //  a6
	write_cmos_sensor(0x372d, 0x53);  //  a6
	write_cmos_sensor(0x372e, 0x06);  //  0c
	write_cmos_sensor(0x372f, 0x10);  //  20
	write_cmos_sensor(0x3730, 0x01);  //  02
	write_cmos_sensor(0x3731, 0x06);  //  0c
	write_cmos_sensor(0x3732, 0x14);  //  28
	write_cmos_sensor(0x3736, 0x20);  //  30
	write_cmos_sensor(0x373a, 0x05);  //  0a
	write_cmos_sensor(0x373b, 0x06);  //  0b
	write_cmos_sensor(0x373c, 0x0a);  //  14
	write_cmos_sensor(0x373e, 0x03);  //  06
	write_cmos_sensor(0x375a, 0x06);  //  0c
	write_cmos_sensor(0x375b, 0x13);  //  26
	write_cmos_sensor(0x375d, 0x02);  //  04
	write_cmos_sensor(0x375f, 0x14);  //  28
	write_cmos_sensor(0x3772, 0x23);  //  46
	write_cmos_sensor(0x3773, 0x02);  // 04 
	write_cmos_sensor(0x3774, 0x16);  // 2c 
	write_cmos_sensor(0x3775, 0x12);  //  13
	write_cmos_sensor(0x3776, 0x04);  //  08
	write_cmos_sensor(0x3778, 0x17);  //  
	write_cmos_sensor(0x37a0, 0x44);  //  88
	write_cmos_sensor(0x37a1, 0x3d);  //  7a
	write_cmos_sensor(0x37a2, 0x3d);  //  7a
	write_cmos_sensor(0x37a7, 0x44);  //  88
	write_cmos_sensor(0x37a8, 0x4c);  //  98
	write_cmos_sensor(0x37a9, 0x4c);  //  98
	write_cmos_sensor(0x37aa, 0x44);  //  88
	write_cmos_sensor(0x37ab, 0x2e);  //  5c
	write_cmos_sensor(0x37ac, 0x2e);  //  5c
	write_cmos_sensor(0x37ad, 0x33);  // 55 
	write_cmos_sensor(0x37ae, 0x0d);  //  19
	write_cmos_sensor(0x37af, 0x0d);  //  19
	write_cmos_sensor(0x37b3, 0x42);  //  84
	write_cmos_sensor(0x37b4, 0x42);  //  84
	write_cmos_sensor(0x37b5, 0x31);  //   ; 60
	write_cmos_sensor(0x3768, 0x22);  //  
	write_cmos_sensor(0x3769, 0x44);  //  
	write_cmos_sensor(0x376a, 0x44);  //  
	...
	write_cmos_sensor(0x380c, 0x07);  //  09
	write_cmos_sensor(0x380d, 0x94);  //  79
					  ...
					  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	preview_setting();
	mdelay(10);
	set_mirror_flip(imgsensor.mirror);//add-wujie@20151110
