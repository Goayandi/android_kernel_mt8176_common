...
BOOL CAMERA_Regulator_PowerOnOFF(struct regulator *pwrreg, BOOL IsOn, int vol)
{
	...
	if (IsOn) {		/* Power ON */
		...
		if (ret != 0) {
			PK_ERR("[%s] camera power regulator fails to disable, ret = 0x%x\n",
			       __func__, ret);
			return FALSE;
		}
		voltage_count = regulator_count_voltages(pwrreg);
		...
		PK_DBG("[%s] low_bound_voltage = %d\n",	__func__, low_bound_voltage);
		if (low_bound_voltage <= 0) {
			PK_ERR("[%s] camera power regulator fails to list, low_bound_voltage = %d\n",
				__func__, low_bound_voltage);
			return FALSE;
		}
		ret = regulator_set_voltage(pwrreg, low_bound_voltage, high_bound_voltage);
		if (ret != 0) {
			PK_ERR("[%s] camera power regulator fails to set high_bound_voltage = %d , low_bound_voltage = %d\n",
	...
	for (i = 0; i < CAMERA_GPIO_COUNT; i++) {
		...
		if (gpio_is_valid(gpio[i].num)) {
			...
			PK_WARN("cam_gpio[%d]: \"%s\"(%d) is %s !!\n", i, gpio[i].names, gpio[i].num,
					(gpio[i].num == (-ENOENT) ? "non-used" : "invalid"));
		}
	}
	return ret;
}
