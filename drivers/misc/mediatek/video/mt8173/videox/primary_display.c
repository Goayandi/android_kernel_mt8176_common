...
int primary_suspend_release_fence(void)
{
	...
	for (i = 0; i < HW_OVERLAY_COUNT; i++) {
		...
		DISPMSG("mtkfb_release_layer_fence  session=0x%x,layerid=%d end\n", session, i);
	}
	return 0;
}
...
int primary_display_suspend(void)
{
	...
	DISPFUNC();
	#if 0
	DISP_CPU_REG_SET(ddp_addr_convert_pa2va(0x1401b178),0x00000000);
	DISP_CPU_REG_SET(ddp_addr_convert_pa2va(0x1401c178),0x00000000);
	DISP_CPU_REG_SET(ddp_addr_convert_pa2va(0x1401b17c),0x00200040);
	DISP_CPU_REG_SET(ddp_addr_convert_pa2va(0x1401c17c),0x00200040);
	msleep(20);
	printk("set test parttern!\n");
	#endif
	//add-test
	w100_suspend = 1;
	printk("[primary_display_suspend]w100 test w100_suspend = 1\n");
	//end
	...
	disp_lcm_suspend_power(pgc->plcm);//wisky-lxh@20170524,power off lcd
	return ret;
}
...
int primary_display_resume(void)
{
	...
	printk("[primary_display_suspend]w100 test w100_suspend = 0\n");
	if(w100_suspend)
	disp_lcm_resume_power(pgc->plcm);//wisky-lxh@20170524,power off lcd
	w100_suspend = 0;
...
#if 0	// remove limit @ modify by lijingwen
