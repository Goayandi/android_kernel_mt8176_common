...
//#define DSI_IN_TEST_PATTERN //wisky-lxh@20170515,goto test pattern,send black frame
unsigned int LCD_STATUS;
EXPORT_SYMBOL(LCD_STATUS);
...
int disp_lcm_suspend(disp_lcm_handle *plcm)
{
	...
	if (_is_lcm_inited(plcm)) {
		...
		if (lcm_drv->suspend) {
			LCD_STATUS = 0;
			#ifdef DSI_IN_TEST_PATTERN
		
			msleep(5000);
			printk("set test parttern aa!\n");
			#endif
			lcm_drv->suspend();
		} else {
		...
		//wisky-lxh@20170524,lcm power on,power off
...
int disp_lcm_resume(disp_lcm_handle *plcm)
{
	...
	if (_is_lcm_inited(plcm)) {
		lcm_drv = plcm->drv;
			#ifdef DSI_IN_TEST_PATTERN
			DISP_CPU_REG_SET(ddp_addr_convert_pa2va(0x1401b17c),0x00200000);
			DISP_CPU_REG_SET(ddp_addr_convert_pa2va(0x1401c17c),0x00200000);
			printk("set normal parttern!\n");
			#endif
		//wisky-lxh@20170524,lcm power on,power off	
...
//wisky-lxh@20170524,lcm power on,power off
void disp_lcm_suspend_power(disp_lcm_handle *plcm)
{
	LCM_DRIVER *lcm_drv = NULL;
...
void disp_lcm_resume_power(disp_lcm_handle *plcm)
{
	LCM_DRIVER *lcm_drv = NULL;
