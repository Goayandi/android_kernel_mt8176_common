...
/*Disable ACCDET*/
	...
	if (cur_eint_state == EINT_PIN_PLUG_IN) {
		...
		#endif
		//end-wisky-lxh	
		//wisky-lxh@20161009,rt5645 detect notify
		#if defined(CONFIG_SND_SOC_MT8173_RT5645_MACH) //???
		 mtk_headset_detect(1);
		#endif
		//end-wisky-lxh
	} else {
/*EINT_PIN_PLUG_OUT*/
...
static inline int accdet_setup_eint(struct platform_device *accdet_device)
{
	...
	accdet_eint_type = IRQ_TYPE_LEVEL_LOW;//IRQ_TYPE_LEVEL_HIGH;
	...
	if (node) {
		...
		//wisky-lxh@20151228
		 #ifdef USE_EXT_AMP
		 gpio_spk_en = of_get_named_gpio(node, "extspkamp-gpio", 0);
		 ret = gpio_request_one(gpio_spk_en, GPIOF_OUT_INIT_HIGH,"extspkamp-gpio");
		 if (ret < 0) {
				ACCDET_DEBUG("Unable to request gpio extspkamp-gpio\n");
				return -EINVAL;
		 }
		 ACCDET_DEBUG("[Accdet]gpio_spk_en=%d!\n",gpio_spk_en);
		 #endif
		
	key_pinctrl = devm_pinctrl_get(&accdet_device->dev);//devm_pinctrl_get(dev);
	key_pinctrl_state=pinctrl_lookup_state(key_pinctrl,"default");
	if (IS_ERR(key_pinctrl_state)) {
		ret = PTR_ERR(key_pinctrl_state);
		dev_err(&accdet_device->dev, "fwq Cannot find key pinctrl key_pinctrl_state!\n");
		return ret;
	}
