...
int  gf_ioctl_spi_init_cfg_cmd(struct mt_tl_chip_conf *mcc, unsigned long arg){
    int retval = 0;
    return retval;
}
...
/* gf_spi_setup_conf_factory, configure spi speed and transfer mode in REE mode
  ...
  */
void gf_spi_setup_conf_factory(struct gf_device *gf_dev, u32 speed, enum tl_spi_transfer_mode mode)
{
    struct mt_tl_chip_conf *mcc = &gf_dev->spi_mcc;
    ...
    if ((mode == TL_DMA_TRANSFER) || (mode == TL_FIFO_TRANSFER)) {
        mcc->com_mod = mode;
    } else {
        /* default set to FIFO mode */
        mcc->com_mod = TL_FIFO_TRANSFER;
    }
...
int gf_ioctl_transfer_raw_cmd(struct gf_device *gf_dev, unsigned long arg,unsigned int bufsiz){
...
#if 1
        if (ioc_xraw.len > 32) {
            gf_spi_setup_conf_factory(gf_dev, ioc_xraw.high_time, TL_DMA_TRANSFER);
        } else {
            gf_spi_setup_conf_factory(gf_dev, ioc_xraw.high_time, TL_FIFO_TRANSFER);
        }
#else
        if (ioc_xraw.len > 32) {
            gf_spi_setup_conf_factory(gf_dev, 8,TL_DMA_TRANSFER);
        } else {
            gf_spi_setup_conf_factory(gf_dev, ioc_xraw.high_time,TL_FIFO_TRANSFER);
        }
#endif
 ...
 int gf_spi_read_bytes(struct gf_device *gf_dev, u16 addr, u32 data_len, u8 *rx_buf)
 {
	 ...
	 if ((data_len + 1) > 32) {
		 gf_dev->spi_mcc.com_mod = TL_DMA_TRANSFER;
	 ...
	 *(tmp_buf + 4) = 0xF1;
	 ...
	 if ((data_len + 1) > 32) {
		 gf_dev->spi_mcc.com_mod = TL_FIFO_TRANSFER;
		 spi_setup(gf_dev->spi);
	 }
	 kfree(xfer);
	 if (xfer != NULL)
