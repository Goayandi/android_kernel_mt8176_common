...
extern int DSI_DumpRegisters(DISP_MODULE_ENUM module, int level);
extern void wisky_mipi_vol_check(void);
...
#define GPIO_KEY_UP        (1)
#define GPIO_KEY_DOWN       (0)
static void gpio_key_s1_handler(unsigned long data);
static DECLARE_TASKLET(gpio_key_s1_tasklet, gpio_key_s1_handler, 0);
static unsigned int key_s1_int_pin = 0;
static int key_s1_irq = 0;
static u16 pre_state_s1 = 1;//high level
static int gpio_key_s1_pdrv_probe(struct platform_device *pdev);
static int gpio_key_s1_pdrv_remove(struct platform_device *pdev);
#define GPIO_KEY_S1_CODE KEY_APPSELECT
struct pinctrl *key_s1_pinctrl;
struct pinctrl_state *gpio_key_s1_as_int;
static unsigned int g_key_s1_state = GPIO_KEY_UP;


static void gpio_key_s2_handler(unsigned long data);
static DECLARE_TASKLET(gpio_key_s2_tasklet, gpio_key_s2_handler, 0);
static unsigned int key_s2_int_pin = 0;
static int key_s2_irq = 0;
static u16 pre_state_s2 = 1;//high level
static int gpio_key_s2_pdrv_probe(struct platform_device *pdev);
static int gpio_key_s2_pdrv_remove(struct platform_device *pdev);
#define GPIO_KEY_S2_CODE KEY_F12
struct pinctrl *key_s2_pinctrl;
struct pinctrl_state *gpio_key_s2_as_int;
static unsigned int g_key_s2_state = GPIO_KEY_UP;


static void gpio_key_s3_handler(unsigned long data);
static DECLARE_TASKLET(gpio_key_s3_tasklet, gpio_key_s3_handler, 0);
static unsigned int key_s3_int_pin = 0;
static int key_s3_irq = 0;
static u16 pre_state_s3 = 1;//high level
static int gpio_key_s3_pdrv_probe(struct platform_device *pdev);
static int gpio_key_s3_pdrv_remove(struct platform_device *pdev);
#define GPIO_KEY_S3_CODE KEY_BACK
struct pinctrl *key_s3_pinctrl;
struct pinctrl_state *gpio_key_s3_as_int;
static unsigned int g_key_s3_state = GPIO_KEY_UP;
...
#if 1
static const struct of_device_id gpio_key_s1_of_match[] = {
	{.compatible = "mediatek,gpio-key-s1"},
	{},
};
static struct platform_driver gpio_key_s1_pdrv = {
	.probe = gpio_key_s1_pdrv_probe,
	.remove = gpio_key_s1_pdrv_remove,
	.driver = {
		   .name = "gpio-key-s1",
		   .owner = THIS_MODULE,
		   .of_match_table = gpio_key_s1_of_match,
		   },
};

static int gpio_key_s1_pdrv_remove(struct platform_device *dev)
{
	printk("gpio_key_s1_pdrv_remove\n");
	return 0;
}
...
static void gpio_key_s1_handler(unsigned long data)
{
	...
	cur_state = __gpio_get_value(key_s1_int_pin);
	pre_state_s1 = cur_state;
	
 if (g_key_s1_state == GPIO_KEY_UP) {
 			g_key_s1_state = GPIO_KEY_DOWN;
			irq_set_irq_type(key_s1_irq, IRQ_TYPE_LEVEL_HIGH);
	} else {
			irq_set_irq_type(key_s1_irq, IRQ_TYPE_LEVEL_LOW);
			g_key_s1_state = GPIO_KEY_UP;
	}	
	
	pressed = 1 - g_key_s1_state;
	kpd_print("cur_state=%d,pressed=%d,gpio key state=%s\n", cur_state,pressed,g_key_s1_state?"KEY UP":"KEY DOWN");
	//if (cur_state != pre_state_s1)
	{
		input_report_key(kpd_input_dev, GPIO_KEY_S1_CODE, pressed);
		input_sync(kpd_input_dev);
		kpd_print("report gpio-key-s1 keycode = %u\n", GPIO_KEY_S1_CODE);
	}	
	enable_irq(key_s1_irq);
}

static irqreturn_t key_s1_eint_func(int irq, void *dev_id)
{
kpd_print("%s\n",__func__);
	disable_irq_nosync(key_s1_irq);
	tasklet_schedule(&gpio_key_s1_tasklet);
	return IRQ_HANDLED;
}
...
static int gpio_key_s1_pdrv_probe(struct platform_device *pdev)
{
	...
	key_s1_pinctrl = devm_pinctrl_get(&pdev->dev);
	if (IS_ERR(key_s1_pinctrl)) {
		ret = PTR_ERR(key_s1_pinctrl);
		dev_err(&pdev->dev, "fwq Cannot find gpio key key_s1_pinctrl!\n");
		return ret;
	}

	gpio_key_s1_as_int = pinctrl_lookup_state(key_s1_pinctrl, "gpio_key_s1_as_int");
	if (IS_ERR(gpio_key_s1_as_int)) {
		ret = PTR_ERR(gpio_key_s1_as_int);
		dev_err(&pdev->dev, "fwq Cannot find key pinctrl gpio_key_s1_as_int!\n");
		return ret;
	}
	pinctrl_select_state(key_s1_pinctrl, gpio_key_s1_as_int);	
	
	node = of_find_compatible_node(NULL, NULL, "mediatek,gpio-key-s1");

	key_s1_int_pin = of_get_named_gpio(node, "key_s1_int", 0);
	ret = gpio_request_one(key_s1_int_pin, GPIOF_OUT_INIT_HIGH,
					 "key_s1_int");
	if (ret < 0) {
			printk("Unable to request gpio key_s1_int\n");
	}

	gpio_direction_input(key_s1_int_pin);	
	
	if (node) 
	{
		key_s1_irq = irq_of_parse_and_map(node, 0);
		kpd_print("[gpio-key-s1]key_s1_irq=%d\n", key_s1_irq);
		ret = request_irq(key_s1_irq, key_s1_eint_func,IRQF_TRIGGER_NONE, "key_s1_int", NULL);
		if (ret > 0)
			printk("EINT IRQ LINE NOT AVAILABLE\n");
		else {
			printk("gpio-key-s1 set EINT finished, key_s1_irq=%d\n",key_s1_irq);
		}		
	}
...
static const struct of_device_id gpio_key_s2_of_match[] = {
	{.compatible = "mediatek,gpio-key-s2"},
	{},
};
static struct platform_driver gpio_key_s2_pdrv = {
	.probe = gpio_key_s2_pdrv_probe,
	.remove = gpio_key_s2_pdrv_remove,
	.driver = {
		   .name = "gpio-key-s2",
		   .owner = THIS_MODULE,
		   .of_match_table = gpio_key_s2_of_match,
		   },
};

static int gpio_key_s2_pdrv_remove(struct platform_device *dev)
{
	printk("gpio_key_s2_pdrv_remove\n");
	return 0;
}
...
static void gpio_key_s2_handler(unsigned long data)
{
	...
	cur_state = __gpio_get_value(key_s2_int_pin);
	pre_state_s2 = cur_state;
	
 if (g_key_s2_state == GPIO_KEY_UP) {
 			g_key_s2_state = GPIO_KEY_DOWN;
			irq_set_irq_type(key_s2_irq, IRQ_TYPE_LEVEL_HIGH);
	} else {
			irq_set_irq_type(key_s2_irq, IRQ_TYPE_LEVEL_LOW);
			g_key_s2_state = GPIO_KEY_UP;
	}	
	
	pressed = 1 - g_key_s2_state;
	kpd_print("cur_state=%d,pressed=%d,gpio key state=%s\n", cur_state,pressed,g_key_s2_state?"KEY UP":"KEY DOWN");
	//if (cur_state != pre_state_s2)
	{
		input_report_key(kpd_input_dev, GPIO_KEY_S2_CODE, pressed);
		input_sync(kpd_input_dev);
		kpd_print("report gpio-key-s2 keycode = %u\n", GPIO_KEY_S2_CODE);
	}	
	enable_irq(key_s2_irq);
}

static irqreturn_t key_s2_eint_func(int irq, void *dev_id)
{
printk("key_s2_eint_func\n");
	disable_irq_nosync(key_s2_irq);
	tasklet_schedule(&gpio_key_s2_tasklet);
	return IRQ_HANDLED;
}
...
static int gpio_key_s2_pdrv_probe(struct platform_device *pdev)
{
	...
	key_s2_pinctrl = devm_pinctrl_get(&pdev->dev);
	if (IS_ERR(key_s2_pinctrl)) {
		ret = PTR_ERR(key_s2_pinctrl);
		dev_err(&pdev->dev, "fwq Cannot find gpio key key_s2_pinctrl!\n");
		return ret;
	}

	gpio_key_s2_as_int = pinctrl_lookup_state(key_s2_pinctrl, "gpio_key_s2_as_int");
	if (IS_ERR(gpio_key_s2_as_int)) {
		ret = PTR_ERR(gpio_key_s2_as_int);
		dev_err(&pdev->dev, "fwq Cannot find key pinctrl gpio_key_s2_as_int!\n");
		return ret;
	}
	pinctrl_select_state(key_s2_pinctrl, gpio_key_s2_as_int);	
	
	node = of_find_compatible_node(NULL, NULL, "mediatek,gpio-key-s2");

	key_s2_int_pin = of_get_named_gpio(node, "key_s2_int", 0);
	ret = gpio_request_one(key_s2_int_pin, GPIOF_OUT_INIT_HIGH,
					 "key_s2_int");
	if (ret < 0) {
			printk("Unable to request gpio key_s2_int\n");
	}

	gpio_direction_input(key_s2_int_pin);	
	
	if (node) 
	{
		key_s2_irq = irq_of_parse_and_map(node, 0);
		kpd_print("[gpio-key-s2]key_s2_irq=%d\n", key_s2_irq);
		ret = request_irq(key_s2_irq, key_s2_eint_func,IRQF_TRIGGER_NONE, "key_s2_int", NULL);
		if (ret > 0)
			printk("EINT IRQ LINE NOT AVAILABLE\n");
		else {
			printk("gpio-key-s2 set EINT finished, key_s2_irq=%d\n",key_s2_irq);
		}		
	}
...
static const struct of_device_id gpio_key_s3_of_match[] = {
	{.compatible = "mediatek,gpio-key-s3"},
	{},
};
static struct platform_driver gpio_key_s3_pdrv = {
	.probe = gpio_key_s3_pdrv_probe,
	.remove = gpio_key_s3_pdrv_remove,
	.driver = {
		   .name = "gpio-key-s3",
		   .owner = THIS_MODULE,
		   .of_match_table = gpio_key_s3_of_match,
		   },
};

static int gpio_key_s3_pdrv_remove(struct platform_device *dev)
{
	printk("gpio_key_s3_pdrv_remove\n");
	return 0;
}
...
static void gpio_key_s3_handler(unsigned long data)
{
	...
	cur_state = __gpio_get_value(key_s3_int_pin);
	pre_state_s3 = cur_state;
	
 if (g_key_s3_state == GPIO_KEY_UP) {
 			g_key_s3_state = GPIO_KEY_DOWN;
			irq_set_irq_type(key_s3_irq, IRQ_TYPE_LEVEL_HIGH);
	} else {
			irq_set_irq_type(key_s3_irq, IRQ_TYPE_LEVEL_LOW);
			g_key_s3_state = GPIO_KEY_UP;
	}	
	
	pressed = 1 - g_key_s3_state;
	kpd_print("cur_state=%d,pressed=%d,gpio key state=%s\n", cur_state,pressed,g_key_s3_state?"KEY UP":"KEY DOWN");
	//if (cur_state != pre_state_s3)
	{
		input_report_key(kpd_input_dev, GPIO_KEY_S3_CODE, pressed);
		input_sync(kpd_input_dev);
		kpd_print("report gpio-key-s3 keycode = %u\n", GPIO_KEY_S3_CODE);
	}	
	enable_irq(key_s3_irq);
}

static irqreturn_t key_s3_eint_func(int irq, void *dev_id)
{
printk("key_s3_eint_func\n");
	disable_irq_nosync(key_s3_irq);
	tasklet_schedule(&gpio_key_s3_tasklet);
	return IRQ_HANDLED;
}
...
static int gpio_key_s3_pdrv_probe(struct platform_device *pdev)
{
	...
	key_s3_pinctrl = devm_pinctrl_get(&pdev->dev);
	if (IS_ERR(key_s3_pinctrl)) {
		ret = PTR_ERR(key_s3_pinctrl);
		dev_err(&pdev->dev, "fwq Cannot find gpio key key_s3_pinctrl!\n");
		return ret;
	}

	gpio_key_s3_as_int = pinctrl_lookup_state(key_s3_pinctrl, "gpio_key_s3_as_int");
	if (IS_ERR(gpio_key_s3_as_int)) {
		ret = PTR_ERR(gpio_key_s3_as_int);
		dev_err(&pdev->dev, "fwq Cannot find key pinctrl gpio_key_s3_as_int!\n");
		return ret;
	}
	pinctrl_select_state(key_s3_pinctrl, gpio_key_s3_as_int);	
	
	node = of_find_compatible_node(NULL, NULL, "mediatek,gpio-key-s3");

	key_s3_int_pin = of_get_named_gpio(node, "key_s3_int", 0);
	printk("key_s3_int_pin=%d\n",key_s3_int_pin);
	ret = gpio_request_one(key_s3_int_pin, GPIOF_OUT_INIT_HIGH,
					 "key_s3_int");
	if (ret < 0) {
			printk("Unable to request gpio key_s3_int\n");
	}

	gpio_direction_input(key_s3_int_pin);	
	
	if (node) 
	{
		key_s3_irq = irq_of_parse_and_map(node, 0);
		kpd_print("[gpio-key-s3]key_s3_irq=%d\n", key_s3_irq);
		ret = request_irq(key_s3_irq, key_s3_eint_func,IRQF_TRIGGER_NONE, "key_s3_int", NULL);
		if (ret > 0)
			printk("EINT IRQ LINE NOT AVAILABLE\n");
		else {
			printk("gpio-key-s3 set EINT finished, key_s3_irq=%d\n",key_s3_irq);
		}		
	}
...
#endif
	kpd_print("suspend!! (%d)\n", kpd_suspend);
	//DSI_DumpRegisters(DISP_MODULE_DSIDUAL, 1); //add-wujie
	wisky_mipi_vol_check();
	printk("w100 kpd_pdrv_suspend\n");
	return 0;
}
...
#if 1
	r = platform_driver_register(&gpio_key_s1_pdrv);
	if (r) {
		kpd_info("register gpio_key_s1_pdrv failed (%d)\n", r);
		return r;
	}
	r = platform_driver_register(&gpio_key_s2_pdrv);
	if (r) {
		kpd_info("register gpio_key_s2_pdrv failed (%d)\n", r);
		return r;
	}
	r = platform_driver_register(&gpio_key_s3_pdrv);
	if (r) {
		kpd_info("register gpio_key_s3_pdrv failed (%d)\n", r);
		return r;
	}
#endif	
#ifdef USE_EARLY_SUSPEND
