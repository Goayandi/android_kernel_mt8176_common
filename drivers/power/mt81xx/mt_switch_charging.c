...
static void set_ta_charging_current(void)
{
	...
	if ((real_v_chrA - ta_v_chr_org) > 3000) {
		g_temp_input_CC_value = p_bat_charging_data->ta_ac_9v_input_current;	/* TA = 9V */
...
static void mtk_ta_init(void)
{
	...
	}else if (p_bat_charging_data->ta_12v_support) {//wisky-lxh@20170427,w100 support 12v
		ta_vchr_tuning = false;
		p_bat_charging_data->v_charger_max = 13500; 
		bat_charger_set_hv_threshold(10500000);
	}else {
...
void battery_pump_express_charger_check(void)
{
	if (true == ta_check_chr_type &&
		...
		printk( "Stop battery_pump_express_charger_check, SOC=%d, ta_check_chr_type = %d, charger_type = %d\n",
...
EXPORT_SYMBOL(battery_pump_express_charger_check);
...
//			mtk_ta_retry_increase();	/* increase TA voltage to 12V */
			charger_vol = battery_meter_get_charger_voltage();
			ta_vchr_tuning = true;
		} else if (BMT_status.SOC > p_bat_charging_data->ta_stop_battery_soc) {
