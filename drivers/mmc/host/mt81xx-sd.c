...
#define PAD_CMD_RX_DLY3		  (0x1f << 1)   /* RW */
...
static void msdc_set_mclk(struct msdc_host *host, unsigned char timing, u32 hz)
{
	...
	if (host->sclk <= 52000000) {
		...
		writel(host->saved_tune_para.pad_cmd_tune, host->base + PAD_CMD_TUNE);
	}

	if (timing == MMC_TIMING_MMC_HS400)
		sdr_set_field(host->base + tune_reg,
			      MSDC_PAD_TUNE_CMDRRDLY, host->hs400_cmd_int_delay);
...
static int emmc_tune_response(struct mmc_host *mmc, u32 opcode)
{
	struct msdc_host *host = mmc_priv(mmc);
	u32 cmd_delay = 0;
	...
	if (mmc->ios.timing == MMC_TIMING_MMC_HS200 ||
		...
		sdr_set_field(host->base + tune_reg,
			      MSDC_PAD_TUNE_CMDRRDLY, host->hs200_cmd_int_delay);
	sdr_set_field(host->base + MSDC_IOCON, MSDC_IOCON_RSPL, host->hs200_cmd_resp_sel);
	for (i = 0 ; i < PAD_DELAY_MAX; i++) {
		sdr_set_field(host->base + PAD_CMD_TUNE, PAD_CMD_RX_DLY3, i);
		for (j = 0; j < 3; j++) {
			msdc_send_tuning(mmc, opcode, &cmd_err);
			if (!cmd_err) {
				cmd_delay |= (1 << i);
	...
	sdr_set_field(host->base + PAD_CMD_TUNE, PAD_CMD_RX_DLY3, final_cmd_delay.final_phase);
...
static int msdc_execute_tuning(struct mmc_host *mmc, u32 opcode)
{
	...
	if (host->hs400_mode == false)
		...
		ret = emmc_tune_response(mmc, opcode);
	if (ret == -EIO) {
...
static int msdc_drv_probe(struct platform_device *pdev)
{
	...
	if (!of_property_read_u32(pdev->dev.of_node, "hs200_cmd_int_delay",
				  &host->hs200_cmd_int_delay))
		dev_dbg(&pdev->dev, "host->hs200_cmd_int_delay: %x\n",
				host->hs200_cmd_int_delay);
	if (!of_property_read_u32(pdev->dev.of_node, "hs400_cmd_int_delay",
				  &host->hs400_cmd_int_delay))
		dev_dbg(&pdev->dev, "host->hs400_cmd_int_delay: %x\n",
				host->hs400_cmd_int_delay);
